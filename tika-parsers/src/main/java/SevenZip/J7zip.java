...
public class J7zip {
	...
	static void listing(IInArchive archive, Vector<String> listOfNames,
			boolean techMode) {
			...
			if (listOfNames.contains(item.getName())) {
				if (techMode) {
					...
					System.out.println("Packed Size = "
							+ item.getCompressedSize());
					System.out.println("Modified = " + str_tm);
					System.out.println("   Attributes : "
							+ item.getAttributesString());
					...
					System.out.print(String.format("%13d",
							item.getCompressedSize()));
		...
		if (!techMode) {
			...
			System.out.print(String.format(
					"                    %13d%13d %d files", size, packSize,
					nbFiles));
		}
	}

	public static void testOrExtract(IInArchive archive,
			Vector<String> listOfNames, int mode) throws Exception {
		...
		try {
			...
			if (listOfNames.size() >= 1) {
				...
				for (int i = 0; i < archive.size(); i++) {
					System.out.println(archive.getEntry(i).getName());
					if (listOfNames.contains(archive.getEntry(i).getName())) {
						arrays[len++] = i;
					}
				}
			}
			System.out.println("fund :" + len);
			int res;
			...
			if (res == HRESULT.S_OK) {
				if (extractCallbackSpec.NumErrors == 0)
					...
					System.out.println(" " + extractCallbackSpec.NumErrors
							+ " errors");
			} else {
	...
	public static void main(String[] args) throws Exception {
		...
		case MODE_LISTING:
			...
			break;
		case MODE_TESTING:
			testOrExtract(archive, listOfNames,
