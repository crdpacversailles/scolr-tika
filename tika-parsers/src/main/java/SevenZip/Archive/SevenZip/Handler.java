...
public class Handler implements SevenZip.Archive.IInArchive {
	...
	public int Open(IInStream stream, // InStream *stream
	...
	public int Extract(int[] indices, int numItems, int testModeSpec,
			IArchiveExtractCallback extractCallbackSpec, String tempFileDirPath) throws IOException {
		...
		for (int ii = 0; ii < numItems; ii++) {
			...
			{
				/*
				 * #ifdef _7Z_VOL // const CRef &ref = ref2.Refs[ri]; const CRef
				 * &ref = _refs[ref2Index];
				 * 
				...
				int folderIndex = database.FileIndexToFolderIndexMap
						.get(fileIndex);
				if (folderIndex == SevenZip.Archive.SevenZip.InArchive.kNumNoIndex) {
					extractFolderInfoVector.add(new ExtractFolderInfo(
							...
							// #endif
							fileIndex,
							SevenZip.Archive.SevenZip.InArchive.kNumNoIndex));
					continue;
				}
				...
				for (int index = efi.ExtractStatuses.size(); index <= fileIndex
						- startIndex; index++) {
					// UInt64 unPackSize = _database.Files[startIndex +
					// index].UnPackSize;
		...
		for (int i = 0; i < extractFolderInfoVector.size(); i++, currentImportantTotalUnPacked += totalFolderUnPacked) {
			...
			int res = extractCallback
					.SetCompleted(currentImportantTotalUnPacked);
			if (res != HRESULT.S_OK)
				return res;
			...
			// #else
					0,
					// #endif
					startIndex, efi.ExtractStatuses, extractCallback, testMode, tempFileDirPath);
			...
			LocalCompressProgressInfo localCompressProgressSpec = new LocalCompressProgressInfo();
			...
			int packStreamIndex = database.FolderStartPackStreamIndex
					.get(folderIndex); // CNum
			long folderStartPackPos = database.GetFolderStreamPos(folderIndex,
					0);

			/*
			 * #ifndef _NO_CRYPTO CMyComPtr<ICryptoGetTextPassword>
			 * getTextPassword; if (extractCallback)
			...
			try {
				result = decoder.Decode(
						...
						packStreamIndex, folderInfo, outStream,
						compressProgress
				// #ifndef _NO_CRYPTO
				// , getTextPassword
				// #endif
				...
				if (result == HRESULT.S_FALSE) {
					result = folderOutStream
							.FlushCorrupted(IInArchive.NExtract_NOperationResult_kDataError);
					if (result != HRESULT.S_OK)
						return result;
					continue;
				}
	...
	static int GetUInt32FromMemLE(byte[] p, int off) {
		return p[off] | (((int) p[off + 1]) << 8) | (((int) p[off + 2]) << 16)
				| (((int) p[off + 3]) << 24);
	}
					...
					if (methodIsKnown) {
						...
						if (altCoderInfo.MethodID.equals(MethodID.k_LZMA)) {
							if (altCoderInfo.Properties.GetCapacity() >= 5) {
								methodsString += ":";
								int dicSize = GetUInt32FromMemLE(
										altCoderInfo.Properties.data(), 1);
								methodsString += GetStringForSizeValue(dicSize);
							}
						}
						/*
						 * else if (altCoderInfo.MethodID == k_PPMD) { if
						 * (altCoderInfo.Properties.GetCapacity() >= 5) { Byte
						 * order = *(const Byte *)altCoderInfo.Properties;
						 * methodsString += ":o"; methodsString +=
						 * ConvertUInt32ToString(order); methodsString +=
						 * ":mem"; UInt32 dicSize = GetUInt32FromMemLE( ((const
						 * Byte *)altCoderInfo.Properties + 1)); methodsString
						 * += GetStringForSizeValue(dicSize); } } else if
						 * (altCoderInfo.MethodID == k_AES) { if
						 * (altCoderInfo.Properties.GetCapacity() >= 1) {
						 * methodsString += ":"; const Byte *data = (const Byte
						 * *)altCoderInfo.Properties; Byte firstByte = *data++;
						 * UInt32 numCyclesPower = firstByte & 0x3F;
						 * methodsString +=
						 * ConvertUInt32ToString(numCyclesPower); } } else { if
						 * (altCoderInfo.Properties.GetCapacity() > 0) {
						 * methodsString += ":["; for (size_t bi = 0; bi <
						 * altCoderInfo.Properties.GetCapacity(); bi++) { if (bi
						 * > 5 && bi + 1 <
						 * altCoderInfo.Properties.GetCapacity()) {
						 * methodsString += ".."; break; } else methodsString +=
						 * GetHex2(altCoderInfo.Properties[bi]); } methodsString
						 * += "]"; } }
						 */
					} else {
						// TBD methodsString +=
						// altCoderInfo.MethodID.ConvertToString();
					}
				}
			}
			ret = methodsString;
	...
	public SevenZipEntry getEntry(int index) {
		...
		SevenZipEntry entry = new SevenZipEntry(item.name, getPackSize(index2),
				item.UnPackSize, crc, item.LastWriteTime, position,
				item.IsDirectory, item.Attributes, getMethods(index2));
